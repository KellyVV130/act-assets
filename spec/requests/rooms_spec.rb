...
RSpec.describe 'Rooms API', type: :request do
  ...
  describe 'POST /rooms' do
    # valid payload
    let(:valid_attributes) { { name: 'Learn Elm', coords: '119,350,285,350,285,448,119,448' } }
    ...
    context 'when the request is invalid' do
      before { post '/rooms', params: { name: 'Foobar' } }
      ...
      it 'returns a validation failure message' do
        expect(response.body)
          .to match(/Validation failed: Coords can't be blank/)
      end
    end
  end
